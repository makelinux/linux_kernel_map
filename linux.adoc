...
=== TODO
	...
	kernel:__common_interrupt: 32.57 No irq handler for vector
...
=== SMP
	...
	include/linux/smp.h kernel/smp.c arch/x86/kernel/smp.c

	include/linux/smp.h
		smp_send_stop
			smp_ops.stop_other_cpus
		...
		smp_call_function
			...
			smp_call_function_many
				...
				< 
					raise_mce
					__rwmsr_on_cpus
					arm broadcast_tlb_mm_a15_erratum
					drv_write
					scftorture_invoke_one
...
==== CPUHP...
	...
	Call sequence: takedown_cpu, stop_machine_cpuslocked, multi_cpu_stop, local_irq_disable

	Sample challenge:
		smp_call_function -> smp_call_function_many_cond -> send_call_function_single_ipi, csd_lock_wait would wait forever 

		"smp_function_call, Thomas"
		send_call_function_ipi_mask
		on_each_cpu
...
cpu online:
	...
	...
				cpu_stopper_thread < cpu_stop_threads
					...
					ack_state
						MULTI_STOP_PREPARE -> MULTI_STOP_DISABLE_IRQ -> MULTI_STOP_RUN
						atomic_dec_and_test(thread_ack)
						set_state +1
							thread_ack = num_threads
					rcu_momentary_eqs
...
stop_machine* usages
	...
	Srivatsa S. Bhat:
		...
		percpu_rwlock:
			01 Introduce the global reader-writer lock backend
			02 Introduce per-CPU variables for the reader and the writer
			03 Provide a way to define and init percpu-rwlocks at compile time
			04 Implement the core design of Per-CPU Reader-Writer Locks
			05 Make percpu-rwlocks IRQ-safe, optimally
			06 Rearrange the read-lock code to fastpath nested percpu readers
			07 Allow writers to be readers, and add lockdep annotations

		Assorted:
			08 CPU hotplug: Provide APIs to prevent CPU offline from atomic context
			09 CPU hotplug: Convert preprocessor macros to static inline functions
			10 smp, cpu hotplug: Fix smp_call_function_*() to prevent CPU offline properly
			11 smp, cpu hotplug: Fix on_each_cpu_*() to prevent CPU offline properly

			13 sched/migration: Use raw_spin_lock/unlock since interrupts are already disabled
			17 clockevents: Use get/put_online_cpus_atomic() in clockevents_notify()
			22 crypto: pcrypt - Protect access to cpu_online_mask with get/put_online_cpus()

		Use get/put_online_cpus_atomic() to prevent CPU offline:
			kernel:
				12 sched/timer
				14 sched/rt
				15 tick
		...
		stop_machine:
			45 CPU hotplug, stop_machine: Decouple CPU hotplug from stop_machine() in Kconfig
			46 Documentation/cpu-hotplug: Remove references to stop_machine()

		lglock: add read-preference local-global rwlock

		generated by:
		pcregrep  -M 'Subject. \[.*\n\s' a.txt | sed '/Subject/{ N; s/\n / / }'| sort -u 
...
===== sysctl
	...
	system_calls	#337
		...
		???
		x system_call (arch/i386/kernel/entry.S)
...
=== sched
	...
	Scheduler
		...
		schedule kernel/sched.c
			__schedule_loop kernel/sched/core.c
			__schedule 130 LOC
				pre_schedule
...
local_irq_save local_irq_restore, frequent 
				...
				__mutex_unlock_slowpath
					...
					wake_q_add
						__wake_q_add
							wake_q_node
							cmpxchg_relaxed
				x __mutex_fastpath_unlock(&lock->count, __mutex_unlock_slowpath);
				__mutex_unlock_slowpath
					x __mutex_unlock_common_slowpath
						wake_up_process  ▻
				x mutex_clear_owner(lock);
...
==== spinlocks
		...
		spinlock_t
				...
				raw_spin_lock
					_raw_spin_lock
						__raw_spin_lock
							...
							???
								native_queued_spin_lock_slowpath PARAVIRT_SPINLOCKS
				----
					preempt_disable ->
			...
			spin_lock_irqsave lkm2
				RT
				spin_lock include/linux/spinlock_rt.h
					rt_spin_lock ->
	...
	waiman longman
		cpuset-isolation
			sched/isolation: Enable runtime update of housekeeping cpumasks
				+housekeeping_exclude_cpumask
				+cpumask_lock
			sched/isolation: Call sched_tick_offload_init() if housekeeping_overridden is ever set
				+sched_tick_offload_inited
				+sched_tick_offload_init
			sched/isolation: Use RCU to delay successive housekeeping cpumask updates
				synchronize_rcu
				call_rcu(&rcu_gp, rcu_gp_end);
			cgroup: Add a new CSS_KILLED flag
			...
			cgroup/cpuset: Exclude isolated CPUs from housekeeping CPU masks
				...
				update_isolation_cpumasks <- update_unbound_workqueue_cpumask
					+housekeeping_exclude_cpumask
			cgroup/cpuset: Use CPU hotplug code to enable proper transition of isolated CPUs
			...
			cgroup/cpuset: Don't call rebuild_sched_domains*() unnecessarily
			sched/core: Don't check DL BW in sched_cpu_deactivate() if CPU is isolating
				cpuset_cpu_is_isolating
				*sched_cpu_deactivate
	 ...
	 seccomp
	 	do_seccomp
			 ...
			 on_each_cpu_cond_mask
			 	smp_call_function_many_cond
			 ipi_send_cpumask
			 trace_event_raw_event_ipi_send_cpumask
...
CONFIG_NO_HZ_FULL
	task,signal_struct::tick_dep_mask

	tick_nohz_init ->
	tick_nohz_handler -> (9.4 tick_sched_timer, 9.5+ tick_nohz_highres_handler)
	...
	tick_sched_timer➝ tick_nohz_handler
	tick_sched_handle➝
	update_process_times➝
	tick_sched_do_timer➝
	scheduler_tick➝
...
rcu_read_lock){....}-{1:3}, at: debug_show_all_locks+0x3d/0x175
	...
	gpiod_to_irq
	nmap tT4 -A -v 192.168.0.100 # -Taggressive Aggressive
	sudo nmap polite
...
===== RT user tools, rtla
	...
	for t in osnoise timerlat;do for m in top hist; do sudo ./rtla $t $m -D -d 10s -H 1 -c 2; done; done
		...
		osnoise_main (um)
			...
			osnoise_hist_main
				osnoise_hist_apply_config
					...
					-a stop_us
					osnoise_set_stop_us
						"/sys/kernel/tracing/osnoise/stop_tracing_us" -> osnoise_stop_tracing_in
						stop_tracing
	...
	timerlat_hist_main
		timerlat_hist_bpf_main_loop
			timerlat_hist_bpf_pull_data
	...
	nvme_probe
		nvme_mq_ops
			queue_rq -> nvme_queue_rq,
				submit_bio → blk_mq_submit_bio → blk_mq_dispatch_rq_list → .queue_rq (= nvme_queue_rq)
			...
			do_sendfile
				do_splice_direct	lkm2 fs/splice.c
					...
					direct_splice_actor
						do_splice_from
							splice_write
								x generic_splice_sendpage fs/splice.c
									actor = pipe_to_sendpage
										sendpage
											x sock_sendpage  ▻ ./net/socket.c
									splice_from_pipe  ▻
...
== Networking
	...
	ip_rt_do_proc_init
		rt_cache_seq_fops
			fib_trie_seq_show x rt_cache_seq_show
...
=== Sockets
	...
	??
		  - sock_sendpage -> splice_to_socket
		  - tcp_sendpage -> ?
		  - udp_sendpage -> ?
			...
			_inline_copy_to_user
				raw_copy_to_user
					copy_user_generic
						...
						stac (Set AC flag) / clac (Clear AC flag) 
		copy_from_user
			_copy_from_user
...
==== bpf
	...
	__sys_bpf - syscall entry
		...
		bpf_prog_load - load program
  			bpf_check - verifier

	kern_sys_bpf
		bpf_prog_run - execute program
...
patchpal/
	gui/
		...
		command_ai
			...
			parse_result
	backend/
		./src/patchpal/inference/chatbot.py
			main
				WrappedTokenizer
					infer_chatbot
		inference/
...
git commit --fixup=$(git rev-list main --grep="kpm_mcp.py with tests")
...
git remote add $W git://git.kernel.org/pub/scm/linux/kernel/git/trace/linux-$W.git
git worktree list
git fetch $W
git -C ~/linux/main worktree add ../$W $W/for-next

for kdiff3
	~/cmnd/log-preprocess.py
	^1,2,3:x

dconf write /org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/custom2/command '"/home/costa/cmnd/key.sh record"'
dconf write /org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/custom1/command '"/home/costa/cmnd/key.sh stop"'

dconf dump /org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/


p=$(git format-patch -v1 -5 --cover-letter --cover-from-description=subject)
checkpatch.pl $p
m=$(get_maintainer.pl --norolestats --separator=, $p)
git send-email --to="$m" $p

Commit message structure:
	Current state, a problem, issue or bug
	Rationale
	Implementation
	Impact

since 5.15
grep schedule_rtlock /proc/kallsyms
grep -R schedule_rtmutex /proc/kallsyms
...
Python trick
	def key_values(fields):
	    ...
	    return { k: v for f in fields if ':' in f for k, v in [f.split(":", 1)] if k and v }



timerlat hist -D -c 1-3
Error parsing the cpu set 1-3
Invalid -c cpu list
costa-tp ➜  tracing/rtla > L
timerlat hist --duration 1  -E 10 -D -c 1,3  -a 100 
Error parsing the cpu set 1,3
Invalid -c cpu list


MPAM - Arm Memory System Resource Partitioning and Monitoring


=(GOOGLEFINANCE(c6) / IFERROR(INDEX(GOOGLEFINANCE(c6, "price", TODAY()-1), 2, 2), IFERROR(INDEX(GOOGLEFINANCE(c6, "price", TODAY()-2), 2, 2), INDEX(GOOGLEFINANCE(c6, "price", TODAY()-3), 2, 2)))) - 1


=((GOOGLEFINANCE(c9,"price")*GOOGLEFINANCE(d9)) / (IFERROR(INDEX(GOOGLEFINANCE(c9,"price",TODAY()-7),2,2),INDEX(GOOGLEFINANCE(c9,"price",TODAY()-9),2,2)) * IFERROR(INDEX(GOOGLEFINANCE(d9,"price",TODAY()-7),2,2),INDEX(GOOGLEFINANCE(d9,"price",TODAY()-9),2,2))))-1


=((GOOGLEFINANCE(C2,"price")*GOOGLEFINANCE(D2)) / (IFERROR(INDEX(GOOGLEFINANCE(C2,"price",TODAY()-7),2,2),INDEX(GOOGLEFINANCE(C2,"price",TODAY()-9),2,2)) * IFERROR(INDEX(GOOGLEFINANCE(D2,"price",TODAY()-7),2,2),INDEX(GOOGLEFINANCE(D2,"price",TODAY()-9),2,2))))-1




=(googlefinance(b32)/googlefinance(c32) / index(googlefinance(b32, "price", today()-7), 2, 2) * 
 iferror(
   index(googlefinance(c32, "price", today()-7), 2, 2), 
   iferror(
     index(googlefinance(c32, "price", today()-8), 2, 2), 
     index(googlefinance(c32, "price", today()-9), 2, 2)
   )
 ))-1


=(
  googlefinance(b32) / googlefinance(c32) / 
  iferror(
    index(googlefinance(b32, "price", today()-30), 2, 2), 
    iferror(
      index(googlefinance(b32, "price", today()-31), 2, 2), 
      index(googlefinance(b32, "price", today()-32), 2, 2)
    )
  ) * iferror(
    index(googlefinance(c32, "price", today()-30), 2, 2), 
    iferror(
      index(googlefinance(c32, "price", today()-31), 2, 2), 
      index(googlefinance(c32, "price", today()-32), 2, 2)
    )
  )
)-1
...
https://lore.kernel.org/linux-rt-devel/216light.css
/*
...
nvim
	...
	~/.config/nvim/ini.vim
	~/.config/nvim/lua
	~/.config/nvim/lua/quickfix_colors.lua
	~/.config/nvim/lua/smart_grep.lua
	~/.config/nvim/lua/smart_grep_async.lua



add to map:
	dev_get_drvdata     │ device model         │
	netdev_priv         │ networking           │
	seq_printf          │ procfs/seq           │
	devm_kzalloc        │ device resource mgmt │
	atomic_read         │ atomics              │
	rcu_read_unlock     │ RCU                  │
	rcu_read_lock       │ RCU                  │
	sysfs_emit          │ sysfs                │
	module_init         │ modules              │
	copy_to_user        │ user access          │
	kfree_skb           │ networking           │
	free_irq            │ interrupts           │
	dma_alloc_coherent  │ DMA                  │
	debugfs_create_file │ debugfs
